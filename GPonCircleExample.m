...
thetab = [pi/3];
bings = cell(1,length(thetab));
conc = [-2,0]';

for i = 1:length(thetab)
  R = [cos(thetab(i)),-sin(thetab(i));
       sin(thetab(i)),cos(thetab(i))];
  bings{i} = BinghamDistribution(conc(:,i),R);
end
w = ones(1,length(bings))/length(bings);
mixPdfB = HypersphericalMixture(bings,w);
...
fh_f = @(x) 2*mixPdfvM.pdf(x) + 2*mixPdfB.pdf([cos(x);sin(x)]);
f = fh_f(theta);
...
x0 = [1,1,1,0.1];
[~,~,xeopt_vM] = optMOGP(1,1,1,x0,{'vMKernel'},training_points,obs,test_points,gt,plot_traj);
[m_est_vM,covar_vM] = estMOGP(1,1,1,xeopt_vM,{'vMKernel'},training_points,obs,test_points);
...
% close all
[~,~,xeopt_SE] = optMOGP(1,1,1,x0,{'SEKernel'},training_points,obs,test_points,gt,plot_traj);
[m_est_SE,covar_SE] = estMOGP(1,1,1,xeopt_SE,{'SEKernel'},training_points,obs,test_points);
